...
bool HttpSocket::ReceiveHeader(Request& rq){
	...
	for(;;){
		...
		if(!EndHead && rq.HeadPos != String::npos){
			...
			if(has){
				rq.mForm->allTotal += rq.Temp.size();
			}
			printf("request total:%d,ContentLength:%d,temp size:%d\n", total,rq.ContentLength, rq.Temp.size());
			if(rq.ContentLength>0){
				if(rq.ContentLength < 1024){
					printf("request header:%s\n", rq.Header.c_str());
					printf("request temp:%s\n", rq.Temp.c_str());
				}
				if(total >= rq.ContentLength){
					if(rq.Method == POST) {
						if(rq.Temp.size() == rq.ContentLength){
							break;
						}
					} else {
						break;
			...
			if(!has){
				...
				rq.mForm->allTotal += len;
				printf("file write ok!total:%d,%d,bd size:%d,len:%d\n", total,rq.mForm->total,rq.mForm->allTotal,rq.ContentLength);
				if(rq.mForm->allTotal == rq.ContentLength){
					
					break;
				}
			}
			if(rq.ContentLength>0){
				printf("request total:%d,ContentLength:%d\n", total,rq.ContentLength);
				if(total >= rq.ContentLength){
					if(rq.Method == POST) {
						if(rq.Temp.size() == rq.ContentLength){
							break;
...
bool HttpSocket::NewReceiveMultipartFile(const char* buf, size_t len, Request& rq){
	...
	if (rq.hasMultipart){
		...
		if(rq.ContentLength < 900){
			printf("NewReceiveMultipartFile temp:%s\n", rq.mForm->temp.c_str());
		}
		CheckMultipartFileHeader(rq);
	}
...
void HttpSocket::ResponseHeader(Request& rq, Response& rp){
	...
	rp.AddHeader("Content-Length", std::to_string(rp.fileinfo?rp.fileinfo->__stat.st_size:rp.BodySize()));
...
void HttpSocket::ResponseBody(Request& rq, Response& rp){
	if(!rp.fileinfo){
		if(rp.BodySize()!=0){
			...
			// rq.Client->writen(rp.Body, rp.bodySize);
			printf("response _body size:%d\n",  rp._body.size());
			// printf("response _body:%s\n",  rp._body.c_str());
			rq.Client->writen(rp._body.c_str(), rp._body.size());
		}
	}else{
		if(rp.Status != 304){
			char buf2[DownSize];
...
String HttpSocket::render(const Request& rq, const Response& rp){
	String outf = rq.GetParam("o");
	String infstr = rq.GetParam("i");
	String target = rq.GetParam("t");
	String clean = rq.GetParam("c");
	if(infstr.size()>0){
		if(target.size() == 0){
			target = "in";
		}
		...
		String absoluteOutFile; 
		if(ofileIdx>0 && outf.size()>0){
			absoluteOutFile = "\""+outDayDir+"\\"+outf+"\"";
			params = params.Replace("-ofile", absoluteOutFile);
		}
		...
		CloseHandle(logFile);
		if(clean.size()>0){
			if(Path::Exists(_baseDir)){
				Path::Delete(_baseDir);
			}
		}
		if (absoluteOutFile.size()>0) {
			return absoluteOutFile.Replace(WebRoot, "");
		}
	}
	return "";
}
